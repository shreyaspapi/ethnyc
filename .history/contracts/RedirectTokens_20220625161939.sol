...
contract RedirectTokens is SuperAppBase, Ownable {
    ...
    // I think eth should just be an address as there is no point where we want to stream
    address private eth; // accepted token
    ...
    function calculateFlowToken1(int96 _flowRate) view public returns (int96) {
        ...
        uint256 token2Balance = IERC20(eth).balanceOf(address(this));

        if (token1Balance == token2Balance) {
            return _flowRate;
        }

        bool isToken1Greater = usdcx.balanceOf(address(this)) > IERC20(eth).balanceOf(address(this));
    ...
    function calculateFlowToken2(int96 _flowRate) view public returns (int96) {
        ...
        bool isToken2Greater = usdcx.balanceOf(address(this)) < IERC20(eth).balanceOf(address(this));
    ...
    function distributeTokens() external {
        ...
        uint256 amountSwapped = swapToken1(usdcx, IERC20(eth);
        // deposit WETH into aave
        aave.deposit(eth.getUnderLyingToken(), amountSwapped, address(this), 0);
        // borrow MATIC @ 30% of the USDC
        aave.borrow(eth.getUnderlyingToken(), 0.3*amountSwapped, 1, 0, address(this));
