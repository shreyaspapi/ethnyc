...
contract RedirectTokens is SuperAppBase, Ownable {
    ...
    ISuperToken private ownershipToken;
    ...
    constructor(
        ...
        BurnMintSuperToken burnMintToken = BurnMintSuperToken(address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506));

        initialize("Lev super aWETH", "levAWETH", 0x2C90719f25B10Fc5646c82DA3240C76Fa5BcCF34, 0, address(this), "");
    ...
    function distributeTokens() external {
        uint256 amountUSDC = IERC20(usdcx.getUnderlyingToken()).balanceOf(address(this));
        ...
        for (uint i = 0; i < streamAddressesToken1.length; i++) {
            // check how much has been sent in the last period
            address _shareholder = streamAddressesToken1[i];
            // set streaminfo as storage
            StreamInfo storage streamInfo = addressToStreamInfo[_shareholder];
            // get the amount from the mapping
            uint256 amountToAdd = ((block.timestamp-streamInfo.time)*uint256(int(streamInfo.streamRate)) + streamInfo.amount);
            streamInfo.time = block.timestamp;
            streamInfo.amount = 0;
            // convert amountToAdd from USD to ETH
            uint256 percentOfTotal = (amountToAdd*1000/amountUSDC*1000);
            uint256 amountEth = (percentOfTotal * amountSwappedEth)/1000;
            // Send user proportion of the ETH value in our custom superToken
            
            // percentage = 14*1000/100*1000 = (0.14)*1000
            // amountEth = (percentage * amountEth)/1000
            
            // then add to the distribution pool for the IDA
