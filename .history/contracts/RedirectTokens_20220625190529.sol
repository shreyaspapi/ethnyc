...
contract RedirectTokens is SuperAppBase, Ownable {
    ...
    function distributeTokens() external {
        ...
        for (uint i = 0; i < streamAddressesToken1.length; i++) {
            ...
            addressToStreamInfo[_shareholder];
            /*
            timestamp
            amount
            flowRate
            ((block.timestamp-addressToStreamInfo[_shareholder].timestamp)*addressToStreamInfo[_shareholder].flowRate + addressToStreamInfo[_shareholder].amount)
            addressToStreamInfo[_shareholder].timestamp = block.timestamp
            addressToStreamInfo[_shareholder].amount = 0
            addressToStreamInfo[_shareholder].timestamp
            */
