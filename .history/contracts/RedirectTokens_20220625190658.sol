...
contract RedirectTokens is SuperAppBase, Ownable {
    ...
    function distributeTokens() external {
        ...
        aave.deposit(eth.getUnderlyingToken(), amountSwapped, address(this), 0);
        // borrow WETH @ 30% of the USDC
        aave.borrow(eth.getUnderlyingToken(), (3*amountSwapped)/10, 1, 0, address(this));
        ...
        for (uint i = 0; i < streamAddressesToken1.length; i++) {
            ...
            uint256 amountToAdd = ((block.timestamp-addressToStreamInfo[_shareholder].timestamp)*addressToStreamInfo[_shareholder].flowRate + addressToStreamInfo[_shareholder].amount);
            addressToStreamInfo[_shareholder].timestamp = block.timestamp;
            addressToStreamInfo[_shareholder].amount = 0;
